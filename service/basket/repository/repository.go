...
type BasketRepository interface {
	Create(basket basket.Basket) (sql.Result, error)
	Update(basket basket.Basket) (sql.Result, error)
	Delete(id uint) (sql.Result, error)
	List() ([]basket.Basket, error)
	CacheBasket(basket basket.Basket) error
	GetCachedBasket(id uint) (basket.Basket, error)
}
...
func NewBasketRepo(db *sql.DB, redis *redis.Client) BasketRepository {
	return &BasketRepo{
		PostgreSQL: db,
		Redis:      redis,
	}
